...
public class ConfigurationPanel extends JPanel {
	...
	private void init() {
		...
		pNoSharedFoldersSet.add(new JLabel(FileSystemFolderPlugin.messages.getString("ConfigurationPanel.1")));

		bAddFolder = new JButton(FileSystemFolderPlugin.messages.getString("ConfigurationPanel.2"));
		rebuildPanel();
	...
	private void addFolderEntry(String folderPath) {
		if (sharedFolders.size() >= 20) {
			JOptionPane.showMessageDialog(this, FileSystemFolderPlugin.messages.getString("ConfigurationPanel.3"));
			return;
		}
